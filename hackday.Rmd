...
We can also navigate the tree to find the pay of a specific `department`, `organisation`, `unit` and `grade`.

```{r}
tree$`Department for Education`$`Education Funding Agency`$`Education Funding Agency`$SCS2$pay
...
Or we can find out the department with the largest pay. (ffails)

```{r eval = FALSE}
maxGNI <- Aggregate(tree, "pay", max)
...
We aggregate the pay For non-leaves, this will recursively iterate through children, and cache the result in the population field.
...
If we drop the `grade` it might be more informative as not all the departments units have each `grade`.
